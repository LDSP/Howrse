...
$(document).ready(function(){
	if (projectRequest == "elevage%2Fchevaux%2Fcheval"){
		...
		horses = $('nobr>a[href^="/elevage/chevaux/cheval?id="]');
		...
		cheatIco = '<div class="options" style="width: 500px;"><input type="checkbox" name="autoFeed" onclick="options(this);" style="float: left;"><label for="autoFeed" style="float: left;margin: 0px 5px;">АвтоКормежка</label><input type="checkbox" name="autoTrain" onclick="options(this);" style="float: left;"><label for="autoTrain" style="float: left;margin: 0px 5px;">АвтоТренинг</label><input type="checkbox" name="autoSurvey" onclick="options(this);" style="float: left;"><label for="autoSurvey" style="float: left;margin: 0px 5px;">АвтоДосмотр</label><input type="checkbox" name="selectAll" style="float: left;"><label for="selectAll" style="float: left;  margin: 0px 5px;">Выбрать всех</label><a href="javascript:;" class="go" style="background-color: gold;">Погнали</a></div><br/><br/>';
		searchBlock.prepend($(cheatIco).fadeIn(2500));
		if ($.cookie('autoFeed')=="true") $('input[name="autoFeed"]').attr('checked', 'checked');
		if ($.cookie('autoTrain')=="true") $('input[name="autoTrain"]').attr('checked', 'checked');
		if ($.cookie('autoSurvey')=="true") $('input[name="autoSurvey"]').attr('checked', 'checked');
		$('input[name="selectAll"]').change(function(){
			$('.cheatIT').removeAttr('checked');
			if (!compare(horses, $('nobr>a[href^="/elevage/chevaux/cheval?id="]'))){
				horses = $('nobr>a[href^="/elevage/chevaux/cheval?id="]');
				$.each(horses, function(){
					$(this).parent().prepend('<input class="cheatIT" type="checkbox" value="'+this.href+'">');
				});
			}
...
function refreshDisabled(){
	disabled = [];
	$.each($('[class*="action-disabled"]>span[class*="img"]'), function(){ if ($(this).parent().attr('id')) disabled.push($(this).parent().attr('id').match('bouton(.*)-?')[1].toLowerCase())});
	return disabled;
}
...
function care(){	
	...
	if ($.cookie('autoSurvey')=="true" && $.inArray('allaiter' ,disabled)<0 && constructFood()==false && chevalAge<12){
		setTimeout(function(){ new Action.Cheval(null, '/doSuckle', {params : sess1+'='+horseId}).send(); ;}, rTime());time = time+delay+vbr();
	}
	...
	if ($.cookie('autoSurvey')=="true"){
		if ($.inArray('boire' ,disabled)<0){
			setTimeout(function(){ new Action.Cheval(null, '/doDrink', {params : sess1+'='+horseId}).send(); ; },rTime());time = time+delay+vbr();
			if ($.inArray('boire', refreshDisabled())<0){
				setTimeout(function(){ new Action.Cheval(null, '/doDrink', {params : sess1+'='+horseId}).send(); ; },rTime());time = time+delay+vbr();
			}	
		}
		if ($.inArray('caresser-cheval' ,disabled)<0){
			setTimeout(function(){ new Action.Cheval(null, '/doStroke', {params : sess1+'='+horseId}).send(); ; },rTime());time = time+delay+vbr();
		...
		if ($.inArray('etrille' ,disabled)<0){
			setTimeout(function(){ new Action.Cheval(null, '/doGroom', {params : sess1+'='+horseId}).send(); ; },rTime());time = time+delay+vbr();
		...
		if ($.inArray('carotte' ,disabled)<0){
			setTimeout(function(){ new Action.Cheval(null, '/doEatTreat', {params : 'id='+horseId+'&friandise=carotte'}).send(); ; },rTime());time = time+delay+vbr();
			if ($.inArray('carotte' ,refreshDisabled())<0){
				setTimeout(function(){ new Action.Cheval(null, '/doEatTreat', {params : 'id='+horseId+'&friandise=carotte'}).send(); ; },rTime());time = time+delay+vbr();
			}
		}
		if ($.inArray('navet' ,disabled)<0){
			setTimeout(function(){ new Action.Cheval(null, '/doEatTreat', {params : 'id='+horseId+'&friandise=navet'}).send(); ; }, rTime());time = time+delay+vbr();
	...
	if ($.cookie('autoSurvey')=="true" && $.inArray('allaiter' ,disabled)<0 && constructFood()==false && chevalAge<12){
		setTimeout(function(){ new Action.Cheval(null, '/doSuckle', {params : sess1+'='+horseId}).send(); ; },rTime());time = time+delay;
	}
	if (constructFood()!=false && $.cookie('autoFeed')=="true"){
		setTimeout(function(){
			$('#feedingHay').val(constructFood().hay);//
	...
	if ($.cookie('autoSurvey')=="true" && $.inArray('coucher' ,disabled)<0 && $('span.img[style*="coucher-box"]').length==0){
		setTimeout(function(){ new Action.Cheval(null, '/doNight', {params : sess1+'='+horseId+'&'+sess2+'=box'}).send(); ; }, rTime());time = time+delay+vbr();
	}
